...
params.tyrs_end <- 0.5 # Number of Years of Infection 

params.t_end <- params.tyrs_end*365.25 # end time point
    ...
    if (length(locs_possible_identical) != 0) {
      ...
      for (i in 1:length(locs_possible_identical)) {
        ...
        if (ndiff == 0) {
          ...
          #               bool_results <- list(boolNewGenotype = boolNewGenotype, locOldGenotype = locOldGenotype, offspring_genotype = offspring_genotype, offspringFitness_P = offspringFitness_P, offspringFitness_A = offspringFitness_A, offspringFitness = offspringFitness, boolAntigenicChange = boolAntigenicChange)
        }
      }
    }
  }  
...
genotypeVector[1,] <- params.init_genotype
...
genotypeFitness_A <- c()
genotypeFitness_A <- GetGenotypeFitnessA(consensus_genotype, genotypeVector[ , (params.L_P+1):(params.L_P+params.L_PA+params.L_A)])
...
nListByGenotype[,1] <- curr_N_by_genotype
     ...
     nGenotypesCurr <- nrow(nListByGenotype)
     nListByGenotype <- matrix(0, nrow=nGenotypesCurr, ncol=1)
     nListByGenotype[1,1] <- params.N
     
     curr_N_by_genotype <- nListByGenotype[,1]
     ...
     genotypeFitness_A <- c()
     for (i in 1:nGenotypesCurr) {
       genotypeFitness_A[i] <- GetGenotypeFitnessA(consensus_genotype, genotypeVector[i, (params.L_P+1):(params.L_P+params.L_PA+params.L_A)])
     }
...
final_results[[replicate]] <- results
saveRDS(final_results, "results.RData")
}
